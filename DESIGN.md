...
โปรเจกต์นี้ออกแบบเป็น **portfolio / demo เท่านั้น** และกฎที่แท้จริงคือ
**แพลตฟอร์มไม่ถือเงินและไม่จ่ายเงินออก** ไม่ใช่ "ไม่มีเงินจริงอยู่ในระบบเลย"
...
**อัปเดต 2026-08-24 — ช่องทางสลิปเปิดใช้งานจริงบน production แล้ว**

`SLIP_DONATIONS_ENABLED=true` ทำให้หน้าโดเนทมีช่อง "โอนเอง + แนบสลิป" ซึ่งพิมพ์ QR
พร้อมเพย์ของ **สตรีมเมอร์เอง** ผู้ชมโอนเงินจริงเข้าบัญชีสตรีมเมอร์โดยตรง และแพลตฟอร์ม
ไม่เคยแตะเงินก้อนนั้น — ไม่รับไว้ ไม่ถือไว้ ไม่จ่ายต่อ เหตุผล 1-3 ข้างบนจึงยังไม่ถูกละเมิด
(ข้อ 2 และ 3 พูดถึง gateway กับ payout ซึ่งยังเป็น test mode และยังไม่มีทั้งคู่)

สิ่งที่เปลี่ยนคือ **ข้อความบนเว็บ**: "ไม่รับเงินจริง" กลายเป็นคำโกหกทันทีที่ flag นี้เปิด
จึงถูกแทนด้วยประโยคที่แยกสองเส้นทางออกจากกันทุกจุด — gateway = จำลอง, สลิป = เงินจริง
เข้าบัญชีสตรีมเมอร์ ถ้าปิด flag กลับ ข้อความชุดนี้ยังจริงอยู่ (แค่ระมัดระวังเกินจำเป็น)
แต่ถ้าเปิด flag แล้วย้อนข้อความกลับไปเป็นชุดเดิม เว็บจะโกหกคนที่กำลังจะโอนเงิน

**ข้อบังคับที่ต้องทำจริง:**

- README + banner บนหน้าเว็บต้องเขียนชัดว่าเส้นทางไหนเป็นเงินจริง เส้นทางไหนจำลอง
- ห้ามมีข้อความใดทำให้เข้าใจผิดว่าแพลตฟอร์มรับเงินไว้เอง หรือจ่ายเงินออกให้สตรีมเมอร์
...
- [ ] มี banner บอกชัดทุกหน้าที่เกี่ยวกับเงินว่า gateway จำลอง / สลิปเป็นเงินจริงเข้าบัญชีสตรีมเมอร์
