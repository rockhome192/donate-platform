...
> ⚠️ **โปรเจกต์สาธิต — แพลตฟอร์มไม่ถือเงินและไม่จ่ายเงินออก**
> การชำระผ่าน payment gateway **เป็นการจำลองทั้งหมด** (Omise test mode / mock provider)
> ส่วนช่อง "โอนเอง + แนบสลิป" เป็นการโอนพร้อมเพย์ **เข้าบัญชีของสตรีมเมอร์เองโดยตรงด้วยเงินจริง**
> — แพลตฟอร์มไม่เคยรับเงินก้อนนั้นไว้ ไม่ถือไว้ และไม่จ่ายต่อให้ใคร
> เหตุผลที่ออกแบบแบบนี้: การรับโดเนทแล้วจ่ายต่อให้บุคคลที่สามเข้าข่ายธุรกิจที่ ธปท. กำกับ
> และ payment gateway ในไทยเปิด live mode ให้เฉพาะนิติบุคคลที่ผ่าน KYC

ระบบรับโดเนทสำหรับสตรีมเมอร์ พร้อม alert เรียลไทม์บน OBS overlay

<p align="center">
  <img src="docs/slip-demo.gif" width="420" alt="โอนพร้อมเพย์จริง แนบสลิป ระบบตรวจสลิปกับธนาคาร แล้ว alert เด้งขึ้น OBS overlay">
  <br>
  <sub>โดเนทจริง ฿20 ผ่านพร้อมเพย์ → แนบสลิป → ตรวจกับธนาคาร → alert ขึ้น OBS overlay<br>
  GIF ไม่มีเสียง ของจริงมีเสียงอ่านข้อความเป็นภาษาไทยด้วย · QR และเลขบัญชีบนสลิปปิดไว้</sub>
</p>
...
- **ยังไม่ได้เก็บสลิปจากธนาคารต้นทางหลายเจ้า** — การอ่านเลข 4 ตัวท้ายจาก proxy
  ที่ถูก mask พิสูจน์กับสลิปจริงมาแล้ว แต่ยังเป็นธนาคารรับเดียว (004) และ mask
  เขียนโดยแอปของ *ผู้โอน* ธนาคารอื่นจึงยังต่างออกไปได้
...
**รูปอวาตาร์: ไบต์วิ่งผ่านเซิร์ฟเวอร์ และนั่นคือจุดประสงค์** — `POST /api/me/avatar` รับไฟล์ดิบ
เช็ค แล้วเซิร์ฟเวอร์เป็นคนเขียนลง R2 เอง (`putObject` เซ็น SigV4 เองใน `lib/storage.ts`
ไม่ใช้ AWS SDK)

**เดิมไม่ได้เป็นแบบนี้ และเหตุผลที่เปลี่ยนคือแก่นของเรื่อง** เวอร์ชันแรกออก "ตั๋ว" คือ presigned
PUT อายุ 5 นาที ที่เซ็น key + content-type + จำนวนไบต์ไว้ แล้วให้เบราว์เซอร์ยิงเข้า R2 ตรง ๆ
ซึ่งประหยัดกว่าชัด ๆ แต่**สิ่งที่แลกไปคือโอกาสเดียวที่จะได้เห็นไบต์** — content-type กับขนาด
เป็น *คำกล่าวอ้างของผู้อัปโหลด* ลายเซ็นทำให้มันผูกพัน ไม่ได้ทำให้มันจริง เบราว์เซอร์เดา
`File.type` จากนามสกุลไฟล์ ฉะนั้นอะไรก็ตามที่เปลี่ยนชื่อเป็น `.png` ก็ขึ้นไปนอนบนบัคเก็ต
สาธารณะในนามเราได้

ตัวเลขเป็นตัวตัดสิน: avatar จำกัด 2 MB ส่วน Vercel รับ body ได้ 4.5 MB — การ relay ที่
ดูเหมือนทำไม่ได้จึงเหลือที่ว่างอีกเท่าตัว (ถ้าเป็นวิดีโอ 400 MB ก็คงต้องคิดอีกแบบ)

**สิ่งที่ได้มาคือการอ่าน magic bytes** (`lib/images.ts`) — 12 ไบต์แรกของไฟล์บอกว่ามันเป็นอะไร
จริง ๆ แล้วเอาไปเทียบกับ content-type ที่ request อ้าง ไม่ตรง = 415 ไม่ใช่แค่ "ไม่ตรง" นะ
อ่านแล้วไม่รู้จักก็ปฏิเสธเหมือนกัน เพราะ "ไบต์พวกนี้ไม่ใช่ PNG/JPEG/WebP" กับ "นี่ไม่ใช่รูป"
เป็นคำตอบเดียวกันสำหรับของที่กำลังจะถูกเสิร์ฟสาธารณะ WebP เป็นเคสที่ต้องระวัง: มันเป็น RIFF
container ที่มีขนาดไฟล์คั่นอยู่กลาง ต้องเช็คไบต์ 0-3 (`RIFF`) กับ 8-11 (`WEBP`) แบบข้ามช่วงกลาง
ไม่ใช่ prefix ต่อเนื่อง ส่วน JPEG หยุดที่ 3 ไบต์เพราะไบต์ที่ 4 ต่างกันตามชนิด (E0 JFIF,
E1 Exif = รูปจากมือถือทุกใบ) เช็คยาวกว่านั้นคือปฏิเสธไฟล์ที่ถูกต้อง

อีกอย่างที่ได้ฟรี: **ขนาดถูกวัด ไม่ใช่ถูกแจ้ง** ของเดิมทำได้แค่ถามไคลเอนต์ว่าไฟล์ใหญ่เท่าไหร่
แล้วเซ็นคำตอบนั้นไว้ ตอนนี้ไฟล์อยู่ในมือแล้ว

ตรวจกับ bucket จริงแล้ว (2026-08-17 สมัยยังเป็นระบบตั๋ว) **ด้วยสคริปต์ที่ยิง API ตรง ๆ**:
PUT ขึ้น R2 → อ่านกลับมาไบต์ตรงกัน → บันทึกลงแถวโปรไฟล์ → ขึ้นบนหน้าโดเนทสาธารณะ และ
`avatarUrl` นอก host ของเราถูกปฏิเสธ 400 (ไม่งั้นสตรีมเมอร์ชี้รูปไปที่ไหนก็ได้ แล้ว IP ของคนที่
เข้ามาดูรั่วไปหาบุคคลที่สาม) รูปของสตรีมเมอร์คนอื่นบนบัคเก็ตเดียวกันก็ถูกปฏิเสธ 400 เช่นกัน
เส้นทาง PUT ฝั่งเซิร์ฟเวอร์ที่ใช้อยู่ตอนนี้เป็นเส้นเดียวกับที่ TTS ใช้อัปไฟล์เสียงขึ้น R2 ทุกครั้ง
ซึ่งทำงานบน production มาตั้งแต่ 2026-08-19
**ที่ยังไม่เคยทำ: ให้เบราว์เซอร์จริงขับฟอร์มหลังเปลี่ยนมาเป็น one-step**
...
เพราะไม่มีอะไรพัง) บัคเก็ตต้องเปิด public read อย่างเดียว — **ไม่ต้องตั้ง CORS แล้ว** ตั้งแต่ย้าย
มาเป็น one-step เพราะไม่มีเบราว์เซอร์ยิงเข้า R2 อีกต่อไป มีแต่เซิร์ฟเวอร์เรา (กฎ CORS เดิมที่ตั้ง
ไว้ไม่ได้ทำอันตรายอะไร แต่ลบทิ้งได้)

**บัคเก็ตโตอย่างเดียวถ้าไม่ทำอะไร** — `avatarKey()` สุ่มชื่อใหม่ทุกครั้งที่อัป (จำเป็น: ชื่อตายตัว
ต่อสตรีมเมอร์จะติด cache ของ CDN ค้าง เปลี่ยนรูปแล้วไม่ขึ้น) ผลคือไฟล์เก่าค้างอยู่ตลอดไป
แก้ด้วยสองชั้นคู่กัน เพราะชั้นเดียวปิดไม่ครบ:

- **ลบตัวเก่าตอนเปลี่ยนรูป** — `PATCH /api/me/profile` ลบ key เดิมหลัง `update` สำเร็จ
  (หลัง ไม่ใช่ก่อน — การลบย้อนไม่ได้ ส่วน update ยังพลาดได้จาก slug ซ้ำ) ครอบคลุมการใช้งานปกติ
  แต่ **ไม่เห็น**ไฟล์ที่ PUT ขึ้นไปแล้วไม่เคยกด save ซึ่งไม่มีแถวไหนใน DB อ้างถึงเลย
- **กวาดของที่เหลือ** — `pnpm --filter @dp/web sweep:avatars` list บัคเก็ตเทียบกับ
  `Streamer.avatarUrl` ทุกแถว **dry-run เป็นค่าเริ่มต้น** ต้องใส่ `--delete` ถึงจะลบจริง
  ทุกกฎในนั้นเอียงไปทาง *เก็บไว้*: ไฟล์ที่อายุน้อยกว่า 1 ชม. ไม่แตะ (กันเคสอัปแล้วนั่งอ่านฟอร์ม
  อยู่สิบนาทีค่อยกด save) และ key ที่ไม่ตรงรูปแบบที่ `avatarKey()` สร้างก็ไม่แตะ
  รูปที่ถูกลบผิดคือหน้าว่างบนหน้าขอเงิน เรียกคืนไม่ได้ ส่วนรูปที่เก็บเกินคือ 200KB ที่รอบหน้าค่อยกวาด
  (ลองจริงกับบัคเก็ต production แล้ว 2026-09-12: เจอขยะ 3 ไฟล์ 1.9 MB จากรอบเทสวันที่ 17 ส.ค.)

**`tts/` ไม่ต้องมีโค้ดกวาด** — ตั้ง lifecycle rule ใน Cloudflare dashboard เอา
(R2 → เลือกบัคเก็ต → Settings → Object Lifecycle Rules → Add rule, scope ด้วย prefix ได้
สูงสุด 1000 rule ต่อบัคเก็ต) เสียงเป็นของ regenerate ใหม่ได้ และโดเนทที่หลุดจาก `/missed`
ไปแล้วไม่มีใครเล่นซ้ำ — ต่างจาก `avatars/` ที่ lifecycle rule **ใช้ไม่ได้** เพราะมันดูแค่อายุไฟล์
ไม่รู้ว่ารูปไหนยังถูกใช้อยู่ ตั้งไป 30 วันคือลบรูปโปรไฟล์ที่คนใช้อยู่จริงทิ้ง
...
**ผลที่ตามมา (แก้ 24 ส.ค. 2026): เลิกถามเลขบัญชีธนาคารทั้งหมด** หน้าโปรไฟล์เหลือ
**เบอร์พร้อมเพย์ + ชื่อบัญชี** สองช่อง เหตุผลที่ควรเห็นตั้งแต่แรก: **แอปนี้ไม่เคยเผย
เลขบัญชีให้ใคร** เพราะเก็บไว้แค่ 4 ตัวท้าย หน้าโดเนทจึงพิมพ์ได้แค่ `xxx-x-x7788-x`
ซึ่งโอนตามไม่ได้ — สาขา "โอนเข้าเลขบัญชี" จึงเกิดจากการโอนที่นัดกันนอกเว็บนี้เท่านั้น
เราเลยขอข้อมูลที่โดเนทของสตรีมเมอร์คนนั้นไม่มีวันได้ใช้ และแถวเลขบัญชีที่โชว์ให้คนโอนดู
ก็ไม่ได้บอกอะไรที่เอาไปทำอะไรต่อได้ ตัดทิ้งทั้งคู่ เหลือแถว **"เข้าบัญชี {ชื่อ}"**
ซึ่งคนโอนเอาไปเทียบกับชื่อที่แอปธนาคารขึ้นก่อนกดยืนยันได้จริง

คอลัมน์ใน DB และสาขาตรวจเลขบัญชีใน `slip-checks.ts` **ยังอยู่ และไม่ใช่โค้ดตาย**: เมื่อค่า
เป็น null สลิปที่ระบุเลขบัญชีจะถูก **ปฏิเสธ** (`receiver_unconfigured`) ไม่ใช่ปล่อยผ่าน
และไม่ใช่บอกว่าโอนผิดบัญชี — สองอย่างนี้ต้องการคำแนะนำคนละแบบ ถ้าวันหนึ่งเปลี่ยนใจจะเผย
เลขบัญชีเต็ม การตรวจก็เขียนและเทสต์ไว้พร้อมแล้ว
...
**ยิง API จริงแล้ว 21 ส.ค. 2026** — โอนพร้อมเพย์ ฿20 จริง ผ่าน EasySlip ครบทั้งหกชั้น
ลงเป็น `PAID` พร้อม `slipTransRef` เรียบร้อย สิ่งที่กลัวมากที่สุดจึงตกไปด้วย:
**รูปแบบ mask ของเบอร์พร้อมเพย์** — doc เขาเขียนตัวอย่างไว้ว่า `08xxxxxxxx89`
ซึ่งถ้าจริงเป็นแบบนั้น `lastFourDigits` จะอ่านได้ `0889` แล้วชั้น 3 จะกล่าวหาคนที่โอนถูกว่าโอนผิดบัญชี
สลิปจริงใบนั้น**ผ่านชั้น 3** แปลว่า mask จริงเผย 4 ตัวท้ายมาตามปกติ — เลขใน doc เป็นเลขสมมติ

**แต่หลักฐานมีใบเดียว** — mask ถูกเขียนโดยแอปธนาคารฝั่ง**ผู้โอน** สลิปจากธนาคารอื่นจึงยังมีสิทธิ์
หน้าตาต่างออกไป และเพราะ QR ของแอปเป็นพร้อมเพย์เสมอ ทางเลขบัญชีของชั้น 3 จึงยังไม่เคยถูกใช้จริงเลย
`SLIP_DEBUG=true` เปิดค้างไว้ เก็บหลักฐานจากสลิปจริงใบถัด ๆ ไปก่อนจึงจะปิดเคสนี้ได้จริง
...
### สวิตช์เดียวที่ตัดสินว่าเว็บนี้รับเงินจริงหรือไม่
...
**เปิดอยู่บน production ตั้งแต่ 2026-08-24** พร้อมกับแก้ข้อความทั้ง 8 จุดที่เคยเขียนว่า
"ไม่รับเงินจริง" ให้แยกสองเส้นทางออกจากกัน — ถ้าปิด flag กลับ ข้อความชุดนี้ยังจริงอยู่
แต่ถ้าเปิด flag แล้วย้อนข้อความ เว็บจะโกหกคนที่กำลังจะโอนเงิน (DESIGN.md 0)
